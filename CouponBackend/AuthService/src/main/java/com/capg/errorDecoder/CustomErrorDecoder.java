package com.capg.errorDecoder;

import java.io.IOException;

import com.capg.exception.ResourceAlreadyExists;

import com.capg.exception.ValidationFailedException;
...
public class CustomErrorDecoder implements ErrorDecoder {
    ...
    public Exception decode(String methodKey, Response response) {
        if (response.status() == 404) {
            ...
            //return new ResourceNotFound(responseBody);
        } else if(response.status() == 406) {
        	String responseBody = getResponseBody(response);
            return new ResourceAlreadyExists(responseBody);
